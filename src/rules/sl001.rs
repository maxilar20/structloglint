...
pub fn check_sl001(log_call: &LogCall) -> RuleResult {
    if log_call.call.args.len() > 1 {
...
mod tests {
    ...
    fn passes_with_keyword_args_only() {
        let result = check_first_call("log.info('user_logged_in', user_id='u_123')", check_sl001);
        assert_eq!(result.status, Status::Pass, "{}", result.feedback);
    }

    #[test]
    fn passes_with_no_args() {
        let result = check_first_call("log.info('payment_complete')", check_sl001);
        assert_eq!(result.status, Status::Pass, "{}", result.feedback);
    }
    ...
    fn fails_with_one_positional_arg() {
        let result = check_first_call("log.info('user_logged_in', user_id)", check_sl001);
